...
def sync(path,flag):
	...
	if not os.path.isfile(destination):
		display_download_progress(model, fileSize)
		download_file_from_google_drive(file_id, destination,fileSize)
...
def display_download_progress(model, fileSize):
	# gimp.progress_init("Downloading " + model +"(~" + str(fileSize) + "MB)...")
	print("downloading {} ({} MB)".format(model, fileSize))